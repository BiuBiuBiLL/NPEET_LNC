...
class MI:
    ...
    def zip2(*args):
        """
        zip2(x, y) takes the lists of vectors and makes it a list of vectors in
        a joint space
    ...
    def avgdigamma(points, dvec):
        """
        This part finds number of neighbors in some radius in the marginal
        space returns expectation value of <psi(nx)>
        """
        N = len(points)
        ...
        for i in range(N):
            dist = dvec[i]
            # subtlety, we don't include the boundary point, but we are
            # implicitly adding 1 to kraskov def bc center point is included
            num_points = len(tree.query_ball_point(points[i],
                             dist-1e-15, p=float('inf')))
    ...
    def mi_Kraskov(X, k=5, base=np.exp(1), intens=1e-10):
        ...
        ith row of X represents ith dimension of the data,
        e.g. X = [[1.0,3.0,3.0],[0.1,1.2,5.4]], if X has two dimensions and we
        have three samples
        """
    ...
    def mi_LNC(X, k=5, base=np.exp(1), alpha=0.25, intens=1e-10):
        ...
        for point in points:
            ...
            # Calculate covariance matrix of k-nearest neighbor points,
